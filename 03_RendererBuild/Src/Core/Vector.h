...
//向量的加减乘除
template<int n> double operator*(const vec<n>& j, const vec<n>& k)//向量和向量 点积(Dot Projected)
{
	double temp = 0;
...
template<int n> vec<n> operator*(const vec<n>& lhs, const double& rhs) //向量和标量 乘法
{
	vec<n> ret = lhs;
